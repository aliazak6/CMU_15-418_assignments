...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    // run kernel
    double startExecutionTime = CycleTimer::currentSeconds();
    
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();

    double endExecutionTime = CycleTimer::currentSeconds();

    cudaMemcpy(resultarray, device_result, sizeof(float) * N, cudaMemcpyDeviceToHost);
    cudaMemcpy(xarray, device_x, sizeof(float) * N, cudaMemcpyDeviceToHost);
    cudaMemcpy(yarray, device_y, sizeof(float) * N, cudaMemcpyDeviceToHost);
    ...
    printf("Execution: %.3f ms\n", 1000.f*(endExecutionTime - startExecutionTime));
