...
__global__ void Upsweep(int* device_data, int twod, int twod1, int length)
{   
    int index = twod1*(blockIdx.x * blockDim.x + threadIdx.x);
    if(index < length)
        device_data[index+twod1-1] += device_data[index+twod-1];
}
__global__ void Downsweep(int* device_data, int twod, int twod1, int length)
{
    int index = twod1*(blockIdx.x * blockDim.x + threadIdx.x);
    if(index < length){
...
__global__ void Sequential(int* data, int length)
{
    data[length-1] = 0;
}
...
{
    //@warning
        // device_data is in gpu memory so you cannot alter inside host code.
    ...
    for (int twod = 1; twod < roundedLength; twod*=2){
        int twod1 = twod*2;
        Upsweep<<<roundedLength/twod1/256+1,256>>>(device_data,twod,twod1,roundedLength);
    }
    Sequential<<<1,1>>>(device_data, roundedLength);
    // downsweep phase
    for (int twod = roundedLength/2; twod >= 1; twod /= 2){
        int twod1 = twod*2;
        Downsweep<<<roundedLength/twod1/256+1,256>>>(device_data,twod,twod1,roundedLength);
    }
}
...
__global__ void findPeaks(int* device_input, int* device_peaks, int length)
{
    __shared__ int out_index ;
    if(threadIdx.x == 0){
        out_index = 0+blockIdx.x;
    }
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if(index < length - 1 || index > 0){
        if(device_input[index]-device_input[index-1] > device_input[index]-device_input[index+1]){
            device_peaks[out_index] = index;
            out_index++;
        }
    }
}
...
int find_peaks(int *device_input, int length, int *device_output) {
    ...
    findPeaks<<<length/256+1,256>>>(device_input, device_output, length);
    return 0;
}
